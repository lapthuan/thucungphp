...
window.alert_toast = function ($msg = 'TEST', $bg = 'success', $pos = '') {
	var Toast = Swal.mixin({
		...
		customClass: {
			timer: 'my-custom-progress-bar'
		},
		didOpen: () => {
			// Thay đổi màu của thanh tiến trình
			const progressBar = document.querySelector('.swal2-timer-progress-bar');
			if (progressBar) {
				progressBar.style.backgroundColor = $bg == "success" ? '#22c94f' : $bg == "error" ? "#c2442b" : "#ebcd34"; // Màu của thanh tiến trình
			}
		}
	});
	Toast.fire({
			...
			success: function (resp) {
				if (resp) {
					...
					if (resp.status == 'success') {
						...
						var _frm = $('#login-frm')
						var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Tên đăng nhập hoặc mật khẩu không chính xác</div>"
						_frm.prepend(_msg)
