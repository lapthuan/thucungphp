<div>
    <div class="card-header">
        ...
        <div class="card-tools">
            <a href="?page=chinhanh/manage_chinhanh" class="btn btn-flat btn-success rounded"><span
    ...
    <?php
// Truy vấn SQL
$sql = "SELECT machinhanh, tenchinhanh, tinh.tentinh
            FROM public.chinhanh
            JOIN tinh ON tinh.matinh = chinhanh.matinh;";
...
while ($row = pg_fetch_assoc($result)) {
        ?>
        <tr>
            <th scope="row"><?=$row['machinhanh']?></th>
            <td><?=$row['tenchinhanh']?></td>
            <td><?=$row['tentinh']?></td>
            <td>
                <div class="d-flex">
                    <a class="btn btn-success"
                        href="?page=chinhanh/manage_chinhanh&id=<?php echo $row['machinhanh'] ?>">Sửa</a>
                    <a class="btn btn-danger delete_data" href="javascript:void(0)"
                        data-id="<?php echo $row['machinhanh'] ?>">Xóa</a>
                </div>
            </td>
        </tr>
        <?php
...
<script>
		...
		$('.delete_data').click(function(){
			...
			_conf("Bạn có chắc chắn xóa này vĩnh viễn?","delete_chinhanh",[$(this).attr('data-id')])
	...
	function delete_chinhanh($id){

		$.ajax({
			url: _base_url_ + 'classes/Master.php?f=delete_chinhanh',
			method:"POST",
			data:{machinhanh: $id},
			dataType:"json",
			error:err=>{
