<div>
    <div class="card-header">
        ...
        <div class="card-tools">
            <a href="?page=danhmuc/manage_danhmuc" class="btn btn-flat btn-success rounded"><span
...
while ($row = pg_fetch_assoc($result)) {
        ?>
        <tr>
            <th scope="row"><?=$row['madanhmuc']?></th>
            <td><?=$row['tendanhmuc']?></td>
            <td>
                <div class="d-flex">
                    <a class="btn btn-success"
                        href="?page=danhmuc/manage_danhmuc&id=<?php echo $row['madanhmuc'] ?>">Sửa</a>
                    <a class="btn btn-danger delete_data" href="javascript:void(0)"
                        data-id="<?php echo $row['madanhmuc'] ?>">Xóa</a>
                </div>
            </td>
        </tr>
        <?php
...
<script>
		...
		$('.delete_data').click(function(){
			...
			_conf("Bạn có chắc chắn xóa này vĩnh viễn?","delete_danhmuc",[$(this).attr('data-id')])
	...
	function delete_danhmuc($id){
		...
		$.ajax({
			url: _base_url_ + 'classes/Master.php?f=delete_danhmuc',
			method:"POST",
			data:{madanhmuc: $id},
			dataType:"json",
			error:err=>{
